package org.blynder.core.manager;
...
import org.blynder.core.network.url.AppUrlHandlerSetter;
...
import org.blynder.core.view.NotFoundView;
import org.blynder.core.view.Window;
import org.blynder.core.view.WindowSize;

/**
 * 
 * This class is the core of the application, it handles all the initial
 * configuration and the routing of the application. This class is used by the
 * Application static class via its singleton instance.
 *
 */
public class ApplicationManager {
	 ...
	 * class it is inside. That pair is hashed in the method it handles. All the
	 * methods from an url are hashed as the url values.
	 * 
	 */
	...
	/**
	 * 
	 * A map that contains all the filters that are executed for a controller. For
	 * example, imagine that the FooController is filtered with FooFilter1 and
	 * FooFilter2. The map will contain an entry such as { FooFilter.class =
	 * [FooFilter1.class, FooFilter2.class] }.
	 * 
	 */
	...
	/**
	 * 
	 * Sets the setURLStreamHandlerFactory, this will allow the application to
	 * handle all the app:// schema requests. Given this, the application will be
	 * able to handle hrefs, ajax calls and other requests to the schema.
	 * 
	 */
	...
	/**
	 * 
	 * One by one, this method will first find all the needed data, this data are
	 * the classes, controllers, filters and beans. After it has all the needed data
	 ...
	 * autowiring.
	 * 
	 * @throws MapApplicationException If there is a problem in the mapping process.
	 * 
	 */
	protected void mapApplication() throws MapApplicationException {
		try {
			...
			SystemInstanceManager.initInstanceSaver(customClasses, configurator.getAutowiredMapper(),
					configurator.getAutowiredFinder());
	...
	/**
	 * 
	 * Creates the main application window and starts a process to render the
	 * browser for the first time. This is needed for the chromium browser since it
	 * needs to load at least one view before it is ready. That process is taken
	 * care pretty quickly so it won't be noticed by the user.
	 * 
	 */
	...
	/**
	 * 
	 * One of the two entry points of the application manager, the navigate method
	 * without request. This method will delegate into the navigate method with
	 * request. Given that there is no request it will work as a "GET" request.
	 * 
	 * @param url The url the user wants to navigate to.
	 * 
	 * @return The response given by the controller.
	 * 
	 */
	 ...
	 * invoke the NavigationManager that will handle the routing of the URL with the
	 * given request. After the routing occurs it will try to use the response, if
	 * the response is renderizable, for example, html, component or another
	 * supported renderizable it will call the window for its handling. In case it
	 * is not renderizable it will do nothing with the window and will only return
	 * the response, this case is expected to happen if the user is requesting data
	 * from an ApiController via asynchronous calls.
	 * 
	 * @param url     Url that the user wants to navigate to.
	 * 
	 * @param request Request given by the user, might be null.
	 * 
	 * @return The response given by the controller.<br>
	...
	public Response navigate(String url, Request request) {
		try {
			...
			Object[] response = new NavigationManager(url, configurator.getRoutesFinder(), routes, filterManager, request)
					.call();
			...
			if (responseObject.getStatusCode() == 404) {
				responseObject.response(new NotFoundView()).responseType("application/swing");
			}
	 ...
	 * controller wants to do, so it will try to render the view if possible or if
	 * it is not possible it will just do nothing. It is also possible that a filter
	 * or the controller requested a redirect to another path, in that case the app
	 * will be redirected and there will be no render.
	 * 
	 * @param responseArray Composed of a response, a model and a windowSize. The
	 *                      response of the controller. The model that was filled in
	 *                      the controller or an empty one if it wasn't used. The
	 *                      windowSize that will be displayed.
	 * 
	 * @throws IllegalViewException If the view is not supported.
	 * 
	 */
	 ...
	 * 
	 * @param name  Name of the data that will be stored. Will work as the key.
	 * 
	 * @param value Value of the data that will be stoed.
	 * 
	 */
	 ...
	 * 
	 * @param name Key that will be used.
	 * 
	 ...
	 * 
	 * @param name The key that will be removed.
	 * 
	 * @return The value of the data that has been removed.
	 * 
	 */
